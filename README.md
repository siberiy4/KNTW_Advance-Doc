...
コンピュータAのポートN番から通信があったとすると、ルータは対応するK番ポートを用意してあげることで、同じIPにパケットが来てもポートで識別するできます。  
...
CISCO機器には、送信元IPのパケットをすべて許可・拒否する標準ACL、  
送信元IP・プロトコル・送信先IP・ポートなどを指定できる拡張ACLがあります。  
