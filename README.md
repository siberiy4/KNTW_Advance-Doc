...
ネットワークを作る際に使われる技術を少し紹介します。

## DHCP

IPアドレスを自動的に割り当てたり、デフォルトゲートウェイ、使用するDNSなどの情報を伝えるプロトコル。  
スマホやPCをWiFiに接続するとすぐにネットが使えるのは,DHCPを自動で受け取るようにデフォルトで設定されているためです。  
CISCO機器では、DHCPをはくことのできるDHCPサーバになることも、インターフェースごとに設定すればDHCOを受け取るDHCPクライアントになることもできます。  
...
リンクステート型ルーティングプロトコルと呼ばれ、ネットワーク上のすべてのルータの情報を収集し、トポロジー情報、帯域幅情報などをすべて自ルータ内で作成し、ルーティングテーブルを作成する。
最適経路の決定には主に回線の帯域幅の値を使用する。

### BGP

RIPやOSPFに対し、異なる組織ネットワーク間で経路情報をやり取りするルーティングテーブル。
様々な要件で使用されるため、数多くのオプションによる設定を追加することができる